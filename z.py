...
def run_carla_client(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        for episode in range(0, number_of_episodes):
            ...
            # 迭代episode中的每一帧，在这里可以加一个判断来删除掉前多少多少帧。
            ...
            while True:
                ...
                if frame >= 30:
                    ...
                    if flag ==1:
                        RL = DeepQNetwork(n_actions=actions, n_features=point_cloud.color_array.shape[0],
                                          learning_rate=0.01,
                                          reward_decay=0.9,
                                          e_greedy=0.9,
                    ...
                    if a ==1:
                        observation=point_cloud.color_array

                    else:
                        observation_=point_cloud.color_array
                    action = RL.choose_action(point_cloud.color_array)  # 选行为

                    loc2 = np.array(
                        [player_measurements.transform.location.x, player_measurements.transform.location.y])
